...
const CONFIGS = [
  { name: 'sc150', settleClearMs: 150 },
  { name: 'sc110', settleClearMs: 110 },
  { name: 'sc80', settleClearMs: 80 },
];
(async () => {
  ...
  for (const cfg of CONFIGS) {
    ...
    const p0 = bot.piecesPlaced, m0 = bot.mispredicts, r0 = bot.resyncs, l0 = bot.linesEstimate;
    const t0 = Date.now();
    ...
    console.log(`${cfg.name}: ${placed}pc in ${dt.toFixed(1)}s -> ${(placed / dt).toFixed(2)} pps, ` +
                `mispredict ${bot.mispredicts - m0}/${placed}, resync ${bot.resyncs - r0}, lines ${bot.linesEstimate - l0}`);
    await sleep(600);
  }
  await t.close();
  process.exit(0);
