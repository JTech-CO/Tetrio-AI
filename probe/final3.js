...
const CONFIGS = [
  { name: 'pd85', postDropMs: 85 },
  { name: 'pd80-g4-ar6', postDropMs: 80, tapGapMs: 4, afterRotateMs: 6 },
  { name: 'pd95-ctl', postDropMs: 95 },
];
(async () => {
  ...
  for (const cfg of CONFIGS) {
    ...
    const p0 = bot.piecesPlaced, m0 = bot.mispredicts, r0 = bot.resyncs, s0 = bot.stageUps;
    const t0 = Date.now();
    ...
    console.log(`${cfg.name}: ${placed}pc in ${dt.toFixed(1)}s -> ${(placed / dt).toFixed(2)} pps, ` +
                `mispredict ${bot.mispredicts - m0}/${placed}, resync ${bot.resyncs - r0}, stageup ${bot.stageUps - s0}`);
    await sleep(600);
  }
  await t.close();
  process.exit(0);
