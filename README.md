...
콘솔에서 `1/basic`, `2/rapid`, `3/turbo`로 피스 경계에 모드를 전환하고, `single`/`quad`(또는 `싱글`/`쿼드`)로 라인 클리어 방식을 바꾸며, `status`로 통계를 확인합니다. 종료는 `Ctrl+C`입니다.
...
모드(속도)와 별개로, 모든 모드에서 두 방식 중 하나로 플레이합니다. 시작 시 `--strategy`로 정하고, 플레이 중에는 콘솔에서 바로 바꿉니다(다음 배치부터 적용).
...
| `--strategy single/quad` | single | 라인 클리어 방식 |
